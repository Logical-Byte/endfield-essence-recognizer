...
* **避免无意义命名**：避免使用如 `x`, `y`, `i` 等无明确语义的单字母变量名。变量名应能准确描述其承载的数据含义。
* **例外情况**：仅允许在极短的循环索引（如 `for i in range(...)`）或通用的数学背景下使用简写。

### 2. Python 后端规范 (`src/`)

* **文档字符串 (Docstrings)**：所有公有函数、类以及复杂的内部逻辑必须包含完整的 Docstring（建议遵循 Google 风格）。
...
* **逻辑清晰**：如果一段代码逻辑较为复杂（例如涉及复杂的 CV 算法或多层嵌套），则必须配有详细的行内注释。
